...
$rvdata_DIR         = config["rvdata_dir"]
$YAML_DIR           = config["yaml_dir"]
$SCRIPTS_DIR        = config["scripts_dir"]
$rvdata_IGNORE_LIST = config["rvdata_ignore_list"]
$VERBOSE            = config["verbose"]
$MAGIC_NUMBER       = config["magic_number"]

# This is the filename for the export digest.  This file has an entry for
# each RGSS script which has been exported from RMVX.  Each entry has 
# information about the script such as its title in RMVX and the name of
...
# be compared with the modification timestamp for rvdata files to determine
...
# array is used to modify the script title in RMVX's script editor to construct
...
#----------------------------------------------------------------------------
# rvdata_file_exported?: Returns true if the .rvdata file has been exported.
#   filename: The name of the .rvdata file.
#----------------------------------------------------------------------------
def rvdata_file_exported?(filename)
  exported_filename = $PROJECT_DIR + '/' + $YAML_DIR + '/' + File.basename(filename, ".rvdata") + ".yaml"
...
#----------------------------------------------------------------------------
# check_for_rmvx: Checks if rpgxp.exe is running and, if so, exits.  Also
...
#   script: An entry for a script in the loaded Scripts.rvdata file. This
#           is a three element array with the 0th element as the unique id,
#           the 1st element is the script's title in RMVX, and the 3rd 
...
#   title: The title of the script in RMVX's script editor
#----------------------------------------------------------------------------
def fix_name(title)
